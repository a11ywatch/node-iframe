...
const mutateSource = async ({ src = "", key, safeMode }, url, $html, headers) => {
  if (src && src[0] === "/") {
    try {
      ...
      if (res) {
        ...
        $html(key).text(safeMode ? `try {${source}}catch(e){console.error(e)}` : source);
      }
    } catch (e) {
      console.error(e);
    }
...
async function renderHtml(
  { url, baseHref, config, head = {}, safeMode },
  server = false
) {
  if (!url) {
  ...
  if (response) {
    try {
      ...
      for (const com of inlineMutations) {
        ...
        await mutateSource({ key: element, src, safeMode }, url, $html, headers);
